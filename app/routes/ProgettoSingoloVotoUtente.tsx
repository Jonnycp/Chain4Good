...
export default function ProgettoSingoloEnte() {
  ...
  // Gestione Aggiornamento Stato Spesa dal Modale
  const handleUpdateStatus = (id: number, newStatus: StatoSpesa) => {
    setListaSpese(prev => prev.map(item => {
      if (item.id === id) {
        // Se approvo, incremento fittiziamente i voti positivi per dare feedback visivo
        const updatedVotiPositivi = newStatus === 'approvata' 
          ? (item.votiPositivi || 0) + 1 
          : item.votiPositivi;

        // Se rifiuto, incremento fittiziamente i voti negativi
        const updatedVotiNegativi = newStatus === 'rifiutata' 
  ...
  return (
    <div className="min-h-screen bg-white font-sans pb-10 relative">
      ...
      <main className="relative z-10 -mt-8 bg-white rounded-t-[40px] px-6 pt-10 pb-10 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
        ...
        {/* INFO ENTE */}
        <div className="flex items-center gap-3 mb-8 cursor-pointer group" onClick={() => navigate(`/ente-visibile`)}>
