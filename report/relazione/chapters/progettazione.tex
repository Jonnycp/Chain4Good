...
\begin{enumerate}
	\item gli Enti Beneficiari:
	\begin{itemize}
		\item \textbf{Creazione progetto}: L'ente deve poter avviare una nuova iniziativa di raccolta fondi definendone nome, budget target e data di scadenza.
		\item \textbf{Inserimento di una richiesta di spesa}: l'Ente deve poter richiedere il rilascio di una parte dei fondi raccolti, specificando l'importo necessario e allegando obbligatoriamente un preventivo;
		\item \textbf{Caricamento della prova di acquisto}: dopo l'ottenimento dei soldi, l'Ente deve poter caricare la fattura che attesti l'effettivo impiego dell'importo per lo scopo dichiarato;
	...
	\item per i Donatori:
	\begin{itemize}
		\item \textbf{Consultazione dei progetti}:
		\item \textbf{Donazione ad un progetto}: l'utente deve poter selezionare un progetto 
		\item \textbf{Votazione delle richieste di spesa}: il donatore deve poter visualizzare il preventivo allegato e votare (favorevole o contrario) alla richiesta di spesa dell'ente;
		\item \textbf{Consultazione dello storico delle donazioni}:
	\end{itemize}
	
	
	\item per la Piattaforma:
	\begin{itemize}
		\item \textbf{Registrazione delle donazioni}:
		\item \textbf{Blocco dei fondi}: 
		\item \textbf{Gestione del processo di votazione}:
		\item \textbf{Erogazione automatica dei fondi}:
		\item \textbf{Registrazione delle operazioni}:
	\end{itemize}
