...
Un altro ambito di analisi riguarda l'eventualità che l'Ente promotore non utilizzi le risorse raccolte per le finalità dichiarate. Per mitigare tale rischio si è deciso di implementare un meccanismo di custodia decentralizzata dei fondi, in base al quale le risorse non vengono trasferite immediatamente all'Ente al termine della raccolta. Il loro rilascio è subordinato a un meccanismo di valutazione che ne consente lo sblocco solo previo raggiungimento di una maggioranza di voti favorevoli dei donatori sulle singole richieste di spesa presentate dall'Ente.\\
L'introduzione di un meccanismo di votazione ha reso dunque necessario valutare il problema legato alla scarsa partecipazione dei donatori. In particolare, per evitare condizioni di stallo del sistema, si è deciso di adottare una logica di \textit{silenzio-assenso}, progettando la piattaforma in modo tale da approvare automaticamente la richiesta di spesa anche in assenza di voti entro i termini prefissati.

Infine, un ulteriore rischio è rappresentato dall’esposizione dei fondi raccolti alla volatilità intrinseca delle criptovalute. Per ovviare a questo problema, la piattaforma Chain4Good è stata progettata per operare esclusivamente tramite \textit{stablecoin} (nello specifico USDC e EURC), ossia criptovalute progettate per mantenere un valore stabile rispetto a una valuta fiat di riferimento. Tale scelta è motivata dal fatto che, a differenza delle criptovalute (es. ETH in Ethereum), le \textit{stablecoin} permettono di garantire che il potere d'acquisto della donazione rimanga inalterato dal momento del versamento fino all'effettivo utilizzo da parte del beneficiario.
