...
\begin{table}[h]
	...
	\resizebox{\textwidth}{!}{
		\begin{tabular}{p{4cm} p{9cm}}
			...
			\hline
			SIWE & \gls{siwe}: protocollo EIP-4361 di autenticazione adottato per verificare l’identità dell’utente tramite firma crittografica del \textit{wallet}. \\
...
\begin{table}[h]
	...
	\resizebox{\textwidth}{!}{
		\begin{tabular}{p{4cm} p{9cm}}
			...
			\hline
			multer & Libreria di Express.js per gestire l'\textit{upload} dei file tramite form HTML di tipo multipart/form-data. \\
			% migliora
			\hline
		\end{tabular}
	}
...
\begin{table}[h]
	...
	\resizebox{\textwidth}{!}{
		\begin{tabular}{p{4cm} p{9cm}}
			...
			% Mocha
			% È un test runner.
			% Si occupa di eseguire i test, gestire le suite (describe, it), il setup/teardown, e mostrare i risultati.
			%Non si occupa delle asserzioni (cioè dei controlli vero/falso).
			%Chai
			%È una libreria di asserzioni.
			%Fornisce funzioni come expect, assert, should per verificare che i risultati siano quelli attesi.
			%Si integra facilmente con Mocha.
