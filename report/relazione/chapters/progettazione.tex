...
In particolare, i requisiti funzionali definiscono le funzionalità che la piattaforma deve implementare. Essi sono riportati nella Tabella~\ref{tab:requisiti_funzionali}.\\
I requisiti non funzionali rappresentano, invece, tutte le caratteristiche che, pur non essendo funzionalità dirette, il sistema deve garantire. Tali requisiti sono descritti nella Tabella \ref{tab:requisiti_non_funzionali}.
...
\begin{comment}
	La piattaforma proposta si basa sull’adozione del \textit{design pattern} architetturale \gls{mvc} caratterizzato da tre tipi di componenti:
	
	\begin{itemize}
		\item \textit{Model}: incapsula la \textit{business logic} e i dati utili all’applicazione fornendo attraverso la sua interfaccia dei metodi per accedervi e manipolarli;
		
		\item \textit{View}: realizza l’interfaccia grafica, quindi visualizza i dati ottenuti dal Model e si occupa dell’interazione con l’utente;
		
		\item \textit{Controller}: funge da intermediario tra \textit{View} e \textit{Model}, quindi raccoglie i comandi dell’utente provenienti dalla \textit{View} e li attua modificando lo stato degli altri due componenti.
	\end{itemize}
\end{comment}
	


% Nel modello proposto, il \textit{Model} è costituito sia dalla componente \textit{on-chain}, rappresentata dagli \textit{Smart Contract} sviluppati in Solidity e distribuiti sulla blockchain, sia dalla componente \textit{off-chain}, gestita tramite il database MongoDB, utilizzato per l’archiviazione di metadati e informazioni non critiche.\\
% Il livello di \textit{Controller} è implementato dal \textit{back-end} Express, che funge da API Gateway e si occupa della gestione della logica applicativa \textit{off-chain}, dell’autenticazione degli utenti tramite protocollo \gls{siwe} e delle interazioni tra \textit{front-end}, database e blockchain. \\
% Infine, il livello di \textit{View} è rappresentato dal \textit{front-end} sviluppato con React e Tailwind CSS, responsabile della presentazione dei dati e dell’interazione con l’utente finale. 

% Tale architettura consente di combinare i vantaggi della decentralizzazione e dell’immutabilità della blockchain (Web3) con la flessibilità, l’efficienza e la scalabilità delle infrastrutture web tradizionali (Web2).
...
In particolare, le tabelle \ref{tab:frontend_stack} e \ref{tab:backend_stack} riportano rispettivamente gli strumenti utilizzati per l’implementazione della componente \textit{front-end} e \textit{back-end} della piattaforma, con una breve descrizione del loro impiego durante le fasi di sviluppo. Le tecnologie relative alla componente \textit{blockchain} sono invece sintetizzate in Tabella~\ref{tab:blockchain_stack}.
Infine, nella Tabella~\ref{tab:development_tools} sono riportati gli strumenti impiegati per supportare l'organizzazione del lavoro tra i membri del gruppo e per garantire la portabilità del sistema \textit{software}.\\
