...
Il sistema di autenticazione della piattaforma è stato progettato seguendo un approccio \textit{wallet-based}, volto a eliminare l’utilizzo di credenziali tradizionali quali \textit{username} e \textit{password}, coerentemente con il paradigma Web3. L’identità dell’utente è infatti associata in modo univoco al possesso di un \textit{wallet} blockchain. \\
Il processo di \textit{login} ha inizio con la connessione del \textit{wallet} dell’utente, dal quale viene ricavato l’indirizzo pubblico. A partire da tale indirizzo, il \textit{back-end} genera un \textit{nonce}, ovvero un valore casuale e univoco, il quale viene associato all’indirizzo del \textit{wallet} e successivamente restituito al \textit{front-end}. \\
A sua volta, Il \textit{front-end} costruisce un messaggio conforme allo standard \gls{siwe}, includendo il \textit{nonce} ricevuto, e richiede all’utente di firmarlo tramite il proprio \textit{wallet}. La firma crittografica prodotta dimostra il possesso della chiave privata associata all’indirizzo e viene inviata al \textit{back-end} insieme al messaggio firmato. Il \textit{back-end} verifica la validità della firma ricostruendo il messaggio e accertando che: 
\begin{enumerate}
...
\begin{itemize}
	\item \textbf{Approvazione per maggioranza alla scadenza}: la richiesta è approvata se al termine della finestra temporale di votazione, il numero di voti favorevoli raggiunge almeno il 50\% dei voti espressi;
	
	\item \textbf{Approvazione per quorum matematico}: il sistema prevede la chiusura anticipata della votazione qualora il numero di pareri favorevoli raggiunga una soglia tale da rendere l'esito finale non più invertibile, anche nell'ipotesi in cui tutti i restanti aventi diritto esprimessero voto contrario;
	
	\item \textbf{Parità dei voti}: in caso di parità tra voti favorevoli e contrari al termine del periodo di votazione, la richiesta è considerata approvata;

	\item \textbf{Mancata partecipazione}: in assenza di attività di voto entro l'intero periodo di valutazione (pari a tre giorni), il sistema applica il principio del silenzio-assenso, approvando automaticamente la richiesta per non pregiudicare l'avanzamento dei lavori dell'Ente;
...
% la richiesta passa non solo se raggiungo la maggioranza semplice ma anche se il numero di persone che mancano non mi permettono di far cambiare l'esito della votazione
