...
La letteratura converge nel considerare la \textit{blockchain} come una soluzione promettente per il superamento delle criticità strutturali delle piattaforme di \gls{cf} esistenti \cite{shelke2022blockchain}. L'efficacia di tale tecnologia risiede nella capacità di implementare nativamente le proprietà di decentralizzazione, trasparenza e sicurezza (Tabella~\ref{tab:blockchain_vantaggi}), favorendo la transizione da un modello basato sulla fiducia in un intermediario a un paradigma \textit{trustless}, in cui la validità delle operazioni è garantita da protocolli crittografici e dal consenso distribuito. \\
La \textit{blockchain}, infatti, è un registro decentralizzato, condiviso e immutabile, strutturato in una catena sequenziale di blocchi legati tra loro da meccanismi crittografici.
Ciascun blocco contiene un insieme di transazioni, ossia operazioni richieste dagli utenti (come il trasferimento di \textit{asset} digitali), la cui validità e integrità sono garantite da protocolli di consenso distribuito, eseguiti cioè da tutti i nodi all'interno di una rete \gls{p2p} \cite{nakamoto2007bitcoin}.

Nel contesto del \gls{cf}, la natura distribuita del registro (proprietà di decentralizzazione) permette di eliminare i \gls{spof}, tipici delle architetture centralizzate. Parallelamente, la proprietà di trasparenza, garantita alla natura pubblica e verificabile dello stesso, permette ai sostenitori di un'iniziativa di monitorare l’intero ciclo di vita delle donazioni, rafforzando in questo modo la fiducia nelle piattaforme.
