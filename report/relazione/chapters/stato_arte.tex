...
La \textit{blockchain}, infatti, si configura come un registro distribuito, condiviso e immutabile, mantenuto da una rete di nodi interconnessi secondo un’architettura \gls{p2p}. Essa consiste in una catena sequenziale di blocchi legati tra loro da meccanismi crittografici.
...
\end{table}
 ...
 Ciascun blocco contiene un insieme di transazioni (operazioni atomiche come il trasferimento di \textit{asset} digitali), la cui integrità è preservata da protocolli di consenso eseguiti in modo distribuito dalla rete \cite{nakamoto2007bitcoin}. \\
...
Le potenzialità della tecnologia \textit{blockchain} analizzate nel paragrafo precedente hanno dato origine a diverse implementazioni sperimentali in letteratura, volte a prototipare sistemi di \gls{cf} decentralizzati. \\
La maggior parte degli studi esaminati mostra un ricorso diffuso a \textit{Smart Contract} \footnote{programmi immutabili, registrati direttamente sulla \textit{blockchain} ed eseguiti automaticamente al verificarsi di condizioni prestabilite \cite{patil2021blockchain}} per la gestione delle donazioni e a meccanismi di approvazione basati sul consenso dei donatori, per l'approvazione delle richieste di spesa. 

Un esempio significativo è il modello proposto in \cite{patil2021blockchain}, in cui le clausole automatizzate impediscono l'erogazione immediata e integrale dei fondi al beneficiario. Il capitale rimane infatti vincolato nel contratto e viene rilasciato in modo incrementale solo al raggiungimento di obiettivi intermedi stabiliti in fase di creazione del progetto.
La peculiarità di questo modello, tuttavia, si esplica nella presenza di un utente \textit{Admin} a cui è delegata la responsabilità di approvare l’inserimento dei progetti in piattaforma e di autorizzare lo sblocco delle quote raccolte. 
...
Lo \textit{Smart Contract} esegue la transazione direttamente verso il fornitore solo qualora venga raggiunta una maggioranza dei voti superiore al 50\% tra i soli sostenitori del progetto. 
...
%La maggior parte degli studi esaminati mostra un ricorso diffuso a \textit{Smart Contract} per la gestione delle donazioni e a meccanismi di approvazione basati sul consenso dei donatori, per l'approvazione delle richieste di spesa. Un approccio simile è alla base del lavoro di Yadav e Sarasvathi \cite{yadav2020venturing}, in cui però il rilascio dei fondi non è legato esclusivamente a \textit{milestone} temporali, ma è subordinato all'approvazione di una specifica richiesta di spesa avanzata dal promotore del progetto, il quale è tenuto a dettagliare la finalità dell'esborso, l'ammontare richiesto e l'indirizzo pubblico del fornitore destinatario del pagamento. Sebbene nel modello di Patil et al., lo sblocco del capitale è mediata da un utente \textit{Admin}, in \cite{yadav2020venturing}, l'autorizzazione è delegata a un sistema di voto integrato nello \textit{Smart Contract}, che esegue la transazione solo previo raggiungimento di un \textit{quorum} superiore al 50\% dei consensi tra i donatori.  Rispetto al modello precedente, dunque, questa architettura elimina ogni intermediazione in favore di una struttura puramente decentralizzata. 
