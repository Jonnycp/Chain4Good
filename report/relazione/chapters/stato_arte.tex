...
Gli \textit{Smart Contract} sono programmi immutabili, registrati direttamente sulla \textit{blockchain}, che vengono eseguiti automaticamente al verificarsi di condizioni prestabilite  \cite{patil2021blockchain}.

Un esempio significativo è il modello proposto in \cite{patil2021blockchain}, in cui le clausole automatizzate impediscono l'erogazione immediata e integrale dei fondi al beneficiario. Il capitale rimane infatti vincolato nel contratto e viene rilasciato in modo incrementale solo al raggiungimento di obiettivi intermedi (\textit{milestones}) stabiliti in fase di creazione del progetto.
In questo modello, la tutela dei donatori è supportata dalla figura di un utente \textit{Admin}, che funge da supervisore centrale con la responsabilità di approvare l’inserimento dei progetti in piattaforma e convalidare l’effettivo completamento di ogni fase operativa prima di autorizzare lo sblocco delle quote di capitale.

Al contrario, il modello di Yadav e Sarasvathi \cite{yadav2020venturing} elimina ogni intermediazione in favore di una struttura puramente decentralizzata. 
In questo lavoro, infatti, il rilascio dei fondi non è legato esclusivamente a \textit{milestone} temporali, ma è subordinato all'approvazione di una specifica richiesta di spesa avanzata dal promotore del progetto. Quest'ultimo è tenuto a dettagliare la finalità dell’esborso, l’importo richiesto e l’indirizzo pubblico del fornitore destinatario del pagamento. 
L’autorizzazione è delegata a un sistema di voto integrato nello \textit{Smart Contract}, che esegue la transazione solo previo raggiungimento di un \textit{quorum} superiore al 50\% dei consensi tra i soli sostenitori dello specifico progetto. 
