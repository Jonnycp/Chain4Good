...
Un esempio significativo è il modello proposto in \cite{patil2021blockchain}, in cui le clausole automatizzate impediscono l'erogazione immediata e integrale dei fondi al beneficiario. Il capitale rimane infatti vincolato nel contratto e viene rilasciato in modo incrementale solo al raggiungimento di obiettivi prefissati.

Una filosofia analoga, tesa a garantire che i contributori mantengano il controllo sulle quote donate, è alla base del lavoro di Yadav e Sarasvathi \cite{yadav2020venturing}. In questo studio, tuttavia, il rilascio dei fondi non è legato esclusivamente a \textit{milestone} temporali, ma è subordinato all'approvazione di una specifica richiesta di spesa avanzata dal promotore del progetto, il quale è tenuto a dettagliare la finalità dell'esborso, l'ammontare richiesto e l'indirizzo pubblico del fornitore destinatario del pagamento. L'autorizzazione invece è delegata a un sistema di voto integrato nello \textit{Smart Contract}, che esegue la transazione solo previo raggiungimento di un \textit{quorum} superiore al 50\% dei consensi tra i donatori. 

La differenza sostanziale tra i due approcci è che in \cite{patil2021blockchain} 

Mentre il sistema di Patil et al. si configura come un modello supervisionato, in cui una figura centrale di \textit{Admin} funge da garante approvando l'inserimento dei progetti per tutelare i finanziatori da potenziali frodi, l'architettura di Yadav e Sarasvathi elimina ogni intermediazione amministrativa in favore di una struttura puramente decentralizzata e partecipativa.
