...
L’analisi dei rischi è stata condotta al fine di individuare le principali criticità che potrebbero incidere sul corretto svolgimento del progetto e sul funzionamento della piattaforma sviluppata. 

In primo luogo, sono stati considerati i rischi di \textbf{progetto} legati al dominio applicativo del \gls{cf}, tra cui la possibilità che i fondi raccolti non vengano utilizzati per i fini dichiarati dall’ente promotore. Nel sistema proposto, tale rischio viene mitigato mediante un meccanismo di custodia decentralizzata dei fondi, implementato attraverso uno \textit{Smart Contract Vault}, che consente lo sblocco delle risorse esclusivamente a seguito dell’approvazione delle richieste di spesa da parte dei donatori. 
Un ulteriore rischio di progetto è rappresentato dalla volatilità delle criptovalute utilizzate per le donazioni, che può determinare una variazione del controvalore economico dei fondi raccolti rispetto al \textit{budget} inizialmente previsto.
Sebbene tale rischio non possa essere eliminato, la registrazione \textit{on-chain} di tutte le transazioni garantisce trasparenza e tracciabilità sull’andamento delle risorse. 

Accanto a questi aspetti, sono stati considerati rischi di natura tecnologica, legati principalmente all’integrazione tra le componenti \textit{off-chain} e \textit{on-chain} del sistema. Tali rischi sono stati mitigati attraverso una progettazione attenta, l’utilizzo di librerie consolidate e una fase di testing in ambiente di sviluppo. 

Infine, sono stati presi in esame rischi di processo, quali la possibile sottostima dei tempi di sviluppo e l’evoluzione dei requisiti nel corso del progetto, dovuta anche a una conoscenza inizialmente limitata delle tecnologie adottate. 
Per ridurre l’impatto di tali incertezze è stato adottato un approccio di sviluppo incrementale, che ha consentito di adattare progressivamente le soluzioni progettuali sulla base delle evidenze emerse durante l’implementazione. 
