...
% Oltre ai rischi di progetto, sono stati poi presi in esame quelli legati al dominio applicativo del \gls{cf}. 


%E'\ stata, infatti, analizzata la possibilità che i fondi raccolti non fossero utilizzati per i fini dichiarati dall'Ente promotore.  Nel sistema proposto, tale rischio viene mitigato attraverso un meccanismo di custodia decentralizzata dei fondi, implementato mediante uno \textit{Smart Contract} che sblocca le risorse raccolte esclusivamente a seguito dell'approvazione delle richieste di spesa da parte dei donatori.  Un ulteriore rischio di progetto è rappresentato dalla volatilità delle criptovalute utilizzate per le donazioni.

%che può determinare una variazione del controvalore economico dei fondi raccolti rispetto al \textit{budget} inizialmente previsto. Questo rischio viene mitigato attraverso l'utilizzo di \textit{Stable Coin}. 
