...
Data la natura prototipale della piattaforma non è stata condotta una fase di \textit{testing} sul funzionamento sull'intera applicazione. L’attività di verifica è stata effettuata progressivamente durante lo sviluppo: ogni funzionalità è stata testata immediatamente dopo la sua implementazione attraverso test manuali e mediante l’utilizzo di strumenti di sviluppo dedicati, come \textit{Postman} e richieste HTTP da riga di comando (\textit{curl}), per validare il corretto comportamento delle API \textit{backend}. 
